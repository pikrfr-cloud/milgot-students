/** Hebrew UI copy. Single locale — do not add a second language here. */

export const HE = {
  siteName: "מלגות לסטודנטים",
  tagline: "מול הקטלוג שלנו",
  skipToContent: "דילוג לתוכן",
  serviceNotPublicTool: "שירות",

  nav: {
    home: "ראשי",
    profile: "פרופיל",
    results: "הדוח שלי",
    catalog: "קטלוג",
    about: "אודות",
    accessibility: "נגישות",
    terms: "תנאי שימוש",
    privacy: "פרטיות",
    menu: "תפריט",
    close: "סגור",
    myList: "הרשימה שלי",
    updates: "מה עודכן השבוע",
    fastReport: "דוח מהיר",
    chat: "בשיחה קצרה",
  },

  buckets: {
    eligible: "עומדים בתנאי הסף",
    eligibleHint: "התנאים שבקטלוג מתקיימים — לא אישור זכייה ולא הגשה בשמכם.",
    selective: "מיון תחרותי",
    closedCycle: "נסגר למחזור זה",
    closedCycleLong: "נסגר למחזור זה — מתאים למחזור הבא",
    needInfo: "חסר פרט לאישור",
    nearMiss: "כמעט זכאים",
    checkAtInstitution: "לבדוק במוסד",
    checkAtInstitutionLong: "יש לבדוק במוסד/ברשות",
    guide: "מדריך",
    guideLong: "מדריך — לבדוק במוסד/ברשות",
    ineligible: "לא זכאים",
    myList: "הרשימה שלי",
    tips: "טיפים והפניות",
  },

  actions: {
    continue: "המשך",
    back: "הקודם",
    edit: "עריכה",
    print: "הדפסה / שמירה כ‑PDF",
    exportJson: "ייצוא פרופיל (JSON)",
    importJson: "ייבוא פרופיל",
    deleteAll: "מחק את כל הנתונים שלי",
    addToCalendar: "הוספה ליומן (ICS)",
    exportMyListIcs: "ייצוא הרשימה ליומן (ICS)",
    showDetails: "הצג פרטים",
    hideDetails: "הסתר פרטים",
    showIneligible: "הצג את כל מה שנבדק",
    hide: "הסתר",
    tryAgain: "נסו שוב",
    startProfile: "להתחיל את הפרופיל",
    toCatalog: "לעיין בקטלוג",
    toHome: "לעמוד הראשי",
    produceReport: "להפקת דוח הזכאות",
    skip: "לא יודע/ת — דלג",
    showFilters: "סינון",
    fastReport: "דוח מהיר — 5 שאלות",
    completeProfile: "השלמת הפרופיל",
    checkFit: "בדוק אם אתה מתאים",
    chatIntake: "בשיחה קצרה",
    showReportNow: "להראות דוח עכשיו",
  },

  profile: {
    loading: "טוען את הפרופיל…",
    emptyTitle: "אין עדיין פרופיל",
    emptyBody: "כדי לקבל דוח זכאות יש למלא את הפרטים פעם אחת.",
    fillProfile: "למילוי הפרופיל",
    exportWarn:
      "הקובץ מכיל את פרטי הפרופיל שמילאתם (כולל מצב כלכלי ושיוך קהילתי). שמרו אותו במקום בטוח ואל תשלחו אותו בלי צורך.",
    deleteConfirm: "למחוק את כל הנתונים השמורים במכשיר זה? לא ניתן לשחזר.",
    importFail: "לא הצלחנו לקרוא את הקובץ. ודאו שזה ייצוא JSON מהאתר.",
    unknownCity:
      "העיר לא ברשימה — נשמור את מה שהקלדתם. ייתכן שיופיעו פחות התאמות עירוניות.",
  },

  results: {
    title: "דוח הזכאות",
    editProfile: "לערוך פרופיל",
    iphonePrint: "באייפון: שתפו → הדפסה, או בחרו «שמירה כ‑PDF» בתיבת ההדפסה.",
    afterFilter: " (לפי הסינון הנוכחי)",
    printSummary: "סיכום להדפסה",
    urgentTitle: "הדחופות ביותר להגשה",
    urgentNone: "אין כרגע מלגות עם מועד הגשה מפורסם שעדיין פתוח.",
    fillFieldUnlock: "מלא שדה {field} ותפתח ~{n} מלגות",
    catalogAge: "הקטלוג אומת לפני {n} חודשים",
    documentsTitle: "מסמכים מאוחדים",
    documentsHint: "מסמכים שכבר רשומים במלגות הרלוונטיות — לא הומצאו מסמכים.",
    timelineTitle: "מועדי סגירה ב־60 הימים הקרובים",
    timelineNone: "אין מועד סגירה מפורסם ב־60 הימים הקרובים למלגות הרלוונטיות.",
    completeToUnlock: "השלימו את הפרופיל כדי לפתוח עוד מלגות",
    fastPartial: "זה דוח חלקי לפי חמש השאלות. השדות שנשארו פותחים עוד התאמות.",
    nearMissFeatured: "כאן אפשר לפעול: פער שניתן לשנות (התנדבות, היקף, ממוצע).",
    missingAmounts: "{n} מלגות פתוחות בלי סכום מספרי לא נכללו בסה״כ.",
  },

  legal: {
    notOfficial:
      "הדוח אינו החלטת זכאות רשמית של הקרן או של רשות. תמיד יש לאמת באתר המלגה לפני הגשה.",
    localOnly: "הנתונים נשמרים ב־localStorage במכשיר זה בלבד ואינם נשלחים לשרת.",
    deletionRight: "אפשר למחוק את כל הנתונים השמורים במכשיר בכל עת.",
    identityUnpublished:
      "פרטי זהות של מפעיל האתר וכתובת דוא״ל ליצירת קשר עדיין לא פורסמו. אין חשבון משתמש, והנתונים נשארים במכשיר זה בלבד.",
    githubRepoUrl: "https://github.com/pikrfr-cloud/milgot-students",
    githubIssuesUrl: "https://github.com/pikrfr-cloud/milgot-students/issues",
    contactGithub:
      "פניות, דיווחי תקלות ונגישות: Issues במאגר GitHub הציבורי. פרטי זהות של המפעיל עדיין לא פורסמו, ואין דוא״ל באתר.",
  },

  waitlist: {
    title: "עדכונים בדוא״ל",
    body: "בקרוב — עדיין אין שרת ששולח מיילים.",
  },

  catalog: {
    matchableCount: "{n} מלגות להתאמה מול הקטלוג",
    guideCount: "{n} במדריך (דיקן / רשות בלי תנאי סף מאומתים בקטלוג)",
    guideSection: "מדריך — דיקן, עירייה ורשות",
    guideHint:
      "רשומות אלה אינן נספרות כמלגות להתאמה: אין תנאי סף מאומתים בקטלוג. יש לבדוק במקור.",
    duplicate: "כפילות",
    secondaryDeadline: "מקור משני למועד",
  },


  errors: {
    title: "משהו השתבש",
    body: "לא הצלחנו להציג את העמוד. הנתונים השמורים במכשיר לא נמחקו.",
    notFoundTitle: "העמוד לא נמצא",
    notFoundBody: "אין עמוד בכתובת הזו. אפשר לחזור לראשי או לקטלוג.",
  },

  chat: {
    title: "שיחה קצרה",
    intro:
      "כמה שאלות — אחת בכל פעם. אפשר לדלג על כל שאלה. אחרי כמה תשובות אפשר לראות דוח מול הקטלוג.",
    resume: "יש פרופיל שמור במכשיר. אפשר להמשיך מהשאלות שחסרות, או לראות דוח.",
    skip: "דלג",
    showReportNow: "להראות דוח עכשיו",
    continueQuestions: "להמשיך עוד כמה שאלות",
    skipped: "דילוג",
    done: "אלה השאלות הקצרות. אפשר לראות דוח או להשלים בטופס המלא.",
    emptyDone: "לא מולא אף שדה — אפשר להתחיל שוב או למלא בטופס. דילוג לא פוסל מלגה.",
    fullReport: "לדוח המלא",
    completeProfile: "להשלמת הפרופיל בטופס",
    searchInstitution: "חיפוש מוסד",
    searchCity: "הקלידו לחיפוש עיר",
    popularInstitutions: "מוסדות נפוצים",
    moreHint: "הקלידו כדי לחפש ברשימה המלאה.",
    sectorsConfirm: "המשך",
    reportHint: "שדות שדולגו לא פוסלים מלגה — הן יופיעו תחת «חסר פרט לאישור».",
    offerAfterAnswers: "יש כבר כמה תשובות. אפשר לראות דוח עכשיו, או להמשיך.",
    extrasIntro: "עוד כמה שאלות אופציונליות — אפשר לדלג או לראות דוח.",
  },

  whatsapp: {
    introExtra:
      "שלחו «התחל מחדש» כדי להתחיל מחדש. על כל שאלה אפשר «דלג».",
    skipLine: "או שלחו: דלג",
    searchInstitution: "אפשר לחפש בשם, או לבחור מספר:",
    searchCity: "הקלידו את שם העיר (למשל תל אביב, חיפה, באר שבע).",
    multiHint: "אפשר כמה מספרים מופרדים בפסיק (למשל 2,5).",
    unparsed: "לא הבנתי. שלחו מספר מהרשימה, את נוסח התשובה, או דלג.",
    noInstitution: "לא מצאתי מוסד בשם הזה. הקלידו חלק מהשם, בחרו מספר, או דלג.",
    institutionPick: "בחרו מספר מהרשימה, או נסחו שוב:",
    offer: "יש כבר כמה תשובות. אפשר לשלוח «דוח» עכשיו, או להמשיך.",
    tooEarlyReport: "עוד לא מילאתם מספיק שדות לדוח. אפשר לדלג או לענות על השאלה.",
    reportTitle: "סיכום מול הקטלוג (לפי התשובות בשיחה):",
    reportNoneEligible:
      "אין כרגע רשומות שעומדות בתנאי הסף לפי מה שמילאתם. חלק יופיעו תחת «חסר פרט לאישור».",
    topMatches: "התאמות בולטות:",
    fullReportLink: "הדוח המלא באתר:",
    fullReportHint: "לחצו כדי לראות את הדוח עם התשובות שנתתם — בלי שם ובלי טלפון.",
    disclaimer:
      "זה סיכום מול הקטלוג — לא החלטת זכאות של הקרן. יש לאמת באתר המלגה לפני הגשה.",
    sessionPrivacy: "התשובות נשמרות בשיחה הזו בלבד. אין חשבון ואין תשלום.",
    continueAfterReport: "אפשר להמשיך לענות, לשלוח «דוח» שוב, או «התחל מחדש».",
  },

  review: {
    ctaHint: "אפשר להפיק את הדוח עכשיו, או לתקן שדות למטה.",
    filled: "שדות שמולאו",
    skipped: "שדות שדולגו",
    topUnblock: "שלושת השדות שיפתחו הכי הרבה מלגות תחת «חסר פרט»",
  },
} as const;

export type HeKey = typeof HE;
